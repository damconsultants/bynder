...
class AutoAddFormMagento
{
    ...
    public function update_image_attribute($img_json, $product_sku_key, $item_type)
    {
        ...
        try {
            ...
            $pervious_bynder_video = array();
            $pervious_bynder_doc = array();
            $new_image_array = array();
            $new_doc_array = array();
            $new_video_array = array();
            $url_explode =  $diff_video_url_explode =  $diff_doc_url_explode = Null;
            $url_filter = $new_doc_url_filter =  $new_video_url_filter = Null;
            $media_explode = $new_doc_media_explode = $new_video_media_explode = Null;
            ...
            if ($item_type == "image") {
                ...
                if (!empty($doc_value)) {
                    ...
                    $api_response = json_decode($api_call, true);
                    if (isset($api_response['status']) == 1) {
                        $assets_track = $this->datahelper->get_bynder_changemetadata_assets($product_url, $doc_array_merge);
                    }
                    ...
                    $this->_logger->info("Empty doc_value");
                }
                ...
                if (!empty($video_value)) {
                    ...
                    $api_response = json_decode($api_call, true);
                    if (isset($api_response['status']) == 1) {
                        $assets_track = $this->datahelper->get_bynder_changemetadata_assets($product_url, $video_array_merge);
                    }
                    ...
                    $this->_logger->info("Empty video_value");
                }
                //}
                // video section
